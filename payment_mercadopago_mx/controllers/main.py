...
#       id = identificador-de-la-operación
        ...
        if (tx):
            ...
            res = request.env['payment.transaction'].sudo().form_feedback( post, 'mercadopago_mx')

        return res

    @http.route('/payment/mercadopago_mx/ipn/', type='json', auth='none')
    ...
    @http.route('/payment/mercadopago_mx/dpn/', type='http', auth="none")
    ...
    @http.route('/payment/mercadopago_mx/cancel', type='http', auth="none")
