...
class ResCompany(models.Model):
    ...
    def _set_mercadopago_mx_account(self, id, name, value, arg, context=None):
        Acquirer = self.env['payment.acquirer']
        company_id = company = self.env.user.company_id
    ...
    mercadopago_mx_account = fields.Char( copmute=_get_mercadopago_mx_account,
            fnct_inv=_set_mercadopago_mx_account,
            nodrop=True,
            string='MercadoPago Account México',
            help="MercadoPago México username (usually email) for receiving online payments."
        )
