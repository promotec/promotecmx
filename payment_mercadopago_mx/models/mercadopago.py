...
from odoo.addons.payment_mercadopago_mx.controllers.main import MercadoPagoController
...
class AcquirerMercadopago(models.Model):
    ...
    def _get_mercadopago_mx_urls(self, environment):
        """ MercadoPago URLS """
        if environment == 'prod':
            return {
    ...
    def _get_providers(self, context=None):
        ...
        providers.append(['mercadopago_mx', 'MercadoPago México'])
        return providers
    ...
    provider = fields.Selection(selection_add=[('mercadopago_mx', 'MercadoPago México')])
    ...
    def _migrate_mercadopago_mx_account(self, context=None):
        """ COMPLETE ME """
        ...
        #for (company_id, company_mercadopago_account) in res:
            if company_mercadopago_account:
                company_mercadopago_ids = self.search([('company_id', '=', company_id), ('provider', '=', 'mercadopago_mx')], limit=1, context=context)
                if company_mercadopago_ids:
                    ...
                    mercadopago_view = self.env['ir.model.data'].get_object(cr, uid, 'payment_mercadopago_mx', 'mercadopago_mx_acquirer_button')
                    self.create({
                        'name': 'MercadoPago',
    ...
    def mercadopago_mx_compute_fees(self, amount, currency_id, country_id):
    ...
    def mercadopago_mx_dateformat(self, date):
    ...
    def mercadopago_mx_form_generate_values(self, values):
...
#	            },
	            ...
	            "expires": True,
	            "expiration_date_from": self.mercadopago_mx_dateformat( datetime.datetime.now(tzlocal())-datetime.timedelta(days=1) ),
	            "expiration_date_to": self.mercadopago_mx_dateformat( datetime.datetime.now(tzlocal())+datetime.timedelta(days=31) )
                }
    ...
    def mercadopago_mx_get_form_action_url(self):
        environment = 'prod' if self.state == 'enabled' else 'test'
        return self._get_mercadopago_mx_urls(environment)['mercadopago_form_url']

    def _mercadopago_mx_s2s_get_access_token(self, ids, context=None):
        """
        ...
        for acquirer in self.browse( ids, context=context):
            ...
            tx_url = self._get_mercadopago_mx_urls( environment)['mercadopago_rest_url']
    ...
    @api.model
    def mercadopago_mx_get_reference(self, payment_id=None ):
        reference = None
        if (not payment_id):
            return reference
        mps = self.search([('provider','=','mercadopago_mx'),('mercadopago_client_id','!=',False),('mercadopago_secret_key','!=',False)])
        for mp in mps:
            data = mp._mercadopago_mx_get_data(payment_id=payment_id)
            if (data and ('external_reference' in data) ):
                return data['external_reference']
        return reference


    def _mercadopago_mx_get_data(self, payment_id=None, reference=None ):
        data = None
...
class TxMercadoPago(models.Model):
    ...
    def action_mercadopago_mx_check_status( self ):
        data = {}
        _logger.info("action_mercadopago_mx_check_status")
        for tx in self:
            ...
            try:
                data = tx.acquirer_id._mercadopago_mx_get_data(reference=acquirer_reference)
                #_logger.info(data)
                if (data):
                    tx._mercadopago_mx_form_validate(dict(data))
    ...
    # --------------------------------------------------
    @api.model
    def _mercadopago_mx_form_get_tx_from_data(self, data, context=None):
#        reference, txn_id = data.get('external_reference'), data.get('txn_id')
    ...
    def _mercadopago_mx_form_get_invalid_parameters(self, data):
        invalid_parameters = []
...
#charged_back (estado terminal) 	Se realizó un contracargo en la tarjeta de crédito.
    ...
    def _mercadopago_mx_form_validate(self, data):
        #IPN style
        f_data = data
        ...
        if (topic in ["payment"] and payment_id and self.acquirer_id):
            ...
            res = self.acquirer_id._mercadopago_mx_get_data(payment_id=payment_id)
            if (res):
                f_data.update( res )
            external_reference = f_data.get('external_reference')
        elif (external_reference):
            #DPN based on payment id, preferred...
            res = self.acquirer_id._mercadopago_mx_get_data(reference=external_reference)
            if (res):
                f_data.update( res )
        ...
        _logger.info("_mercadopago_mx_form_validate: external_reference: " +str(external_reference))
    ...
    def _mercadopago_mx_try_url(self, request, tries=3, context=None):
    ...
    def _mercadopago_mx_s2s_send(self, values, cc_values, context=None):
        """
         .. versionadded:: pre-v8 saas-3
         .. warning::
        ...
        result = self._mercadopago_mx_try_url(request, tries=3, context=context)
        return (tx_id, result)

    def _mercadopago_mx_s2s_get_invalid_parameters(self, tx, data, context=None):
        """
         .. versionadded:: pre-v8 saas-3
         .. warning::
    ...
    def _mercadopago_mx_s2s_validate(self, tx, data, context=None):
        """
         .. versionadded:: pre-v8 saas-3
         .. warning::
    ...
    def _mercadopago_mx_s2s_get_tx_status(self, tx, context=None):
        """
         .. versionadded:: pre-v8 saas-3
         .. warning::
        ...
        data = self._mercadopago_mx_try_url(request, tries=3, context=context)
